...
theorem zero_succ_limit (x : Ordinal) : x = zero ∨ (∃ y, x = succ y) ∨ IsLimit x := by
  by_cases h0 : x = zero
  · exact Or.inl h0
...
theorem lt_succ_self (x : Ordinal) : x < succ x := by
  revert x
  refine Quotient.ind ?_
  intro a
  exact PreOrd.Mem_self_succ a

theorem inductionOn {p : Ordinal → Prop} (o : Ordinal)
  (h : ∀ α, (∀ β < α, p β) → p α) : p o :=
  WellFounded.induction well_founded_lt o h

theorem limitInductionOn {p : Ordinal → Prop} (o : Ordinal)
  (hzero : p zero)
...
def recOn {motive : Ordinal → Sort u} (o : Ordinal)
  (F : ∀ α, (∀ β < α, motive β) → motive α) : motive o :=
  WellFounded.fix well_founded_lt F o

noncomputable def limitRecOn {motive : Ordinal → Sort u} (o : Ordinal)
  (hzero : motive zero)
