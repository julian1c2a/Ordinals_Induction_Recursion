import OrdinalsInductionRecursion.Arith
...
theorem pow_respects {a1 a2 b1 b2 : PreOrd} (ha : Equiv a1 a2) (hb : Equiv b1 b2) : Equiv (pow a1 b1) (pow a2 b2) := by
  ...
  · have h_a2 : Equiv a2 zero := Equiv_trans (Equiv_symm ha) h_a1
    rw [if_pos h_a1, if_pos h_a2]
    by_cases h_b1 : Equiv b1 zero
    · have h_b2 : Equiv b2 zero := Equiv_trans (Equiv_symm hb) h_b1
      rw [if_pos h_b1, if_pos h_b2]
      exact Equiv_refl _
    · have h_b2 : ¬Equiv b2 zero := fun h => h_b1 (Equiv_trans hb h)
      rw [if_neg h_b1, if_neg h_b2]
      exact Equiv_refl _
  · have h_a2 : ¬Equiv a2 zero := fun h => h_a1 (Equiv_trans ha h)
