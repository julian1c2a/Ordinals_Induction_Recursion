...
def add (a : PreOrd.{u}) : PreOrd.{u} → PreOrd.{u}
  | zero => a
  | succ b => succ (add a b)
  | sup f => union a (sup (fun n => add a (f n)))
...
theorem add_mono_left {a1 a2 : PreOrd.{u}} (h : Subset a1 a2) (b : PreOrd.{u}) : Subset (add a1 b) (add a2 b) := by
  ...
  | sup f ih => exact union_subset (Subset_trans h (subset_union_left _ _)) (Subset_trans (Subset.sup_subset fun n => Subset_sup (ih n) n rfl) (subset_union_right _ _))
...
mutual
  ...
  def add_mono_right_mem (a : PreOrd.{u}) {b1 b2 : PreOrd.{u}} (h : Mem b1 b2) : Mem (add a b1) (add a b2) :=
    ...
    | _, _, .mem_sup n hmem => Mem_Subset_trans (.mem_sup n (add_mono_right_mem a hmem)) (subset_union_right _ _)
end

theorem add_subset_add {a1 a2 b1 b2 : PreOrd.{u}} (ha : Subset a1 a2) (hb : Subset b1 b2) : Subset (add a1 b1) (add a2 b2) :=
  Subset_trans (add_mono_left ha b1) (add_mono_right_sub a2 hb)

theorem add_respects {a1 a2 b1 b2 : PreOrd.{u}} (ha : Equiv a1 a2) (hb : Equiv b1 b2) : Equiv (add a1 b1) (add a2 b2) :=
  ⟨add_subset_add ha.left hb.left, add_subset_add ha.right hb.right⟩

/-- Multiplicación de pre-ordinales -/
def mul (a : PreOrd.{u}) : PreOrd.{u} → PreOrd.{u}
  | zero => zero
  | succ b => add (mul a b) a
  | sup f => sup (fun n => mul a (f n))

theorem mul_mono_left {a1 a2 : PreOrd.{u}} (h : Subset a1 a2) (b : PreOrd.{u}) : Subset (mul a1 b) (mul a2 b) := by
  induction b with
...
mutual
  def mul_mono_right_sub (a : PreOrd.{u}) {b1 b2 : PreOrd.{u}} (h : Subset b1 b2) : Subset (mul a b1) (mul a b2) :=
    match b1, b2, h with
  ...
  def mul_mono_right_mem_add (a : PreOrd.{u}) {b1 b2 : PreOrd.{u}} (h : Mem b1 b2) : Subset (add (mul a b1) a) (mul a b2) :=
    match b1, b2, h with
...
theorem mul_respects {a1 a2 b1 b2 : PreOrd.{u}} (ha : Equiv a1 a2) (hb : Equiv b1 b2) : Equiv (mul a1 b1) (mul a2 b2) :=
...
def pow_aux (a : PreOrd.{u}) : PreOrd.{u} → PreOrd.{u}
  | zero => succ zero
  | succ b => mul (pow_aux a b) a
  | sup f => union (succ zero) (sup (fun n => pow_aux a (f n)))

theorem mul_subset_mul {a1 a2 b1 b2 : PreOrd.{u}} (ha : Subset a1 a2) (hb : Subset b1 b2) : Subset (mul a1 b1) (mul a2 b2) :=
  Subset_trans (mul_mono_left ha b1) (mul_mono_right_sub a2 hb)

theorem pow_aux_mono_left {a1 a2 : PreOrd.{u}} (h : Subset a1 a2) (b : PreOrd.{u}) : Subset (pow_aux a1 b) (pow_aux a2 b) := by
  induction b with
  | zero => exact Subset_refl _
  | succ b' ih => exact mul_subset_mul ih h
  | sup f ih => exact union_subset (subset_union_left _ _) (Subset_trans (Subset.sup_subset fun n => Subset_sup (ih n) n rfl) (subset_union_right _ _))

theorem succ_subset_succ {x y : PreOrd.{u}} (h : Subset x y) : Subset (succ x) (succ y) :=
  Subset.succ_subset (Subset_Mem_trans h (Mem_self_succ y))

theorem le_add_zero_left (x : PreOrd.{u}) : Subset x (add zero x) := by
  induction x with
  | zero => exact Subset_refl _
  | succ x' ih => exact succ_subset_succ ih
  | sup f ih => exact Subset_trans (Subset.sup_subset fun n => Subset_sup (ih n) n rfl) (subset_union_right _ _)

theorem le_mul_self_of_ge_one (x a : PreOrd.{u}) (ha : Subset (succ zero) a) : Subset x (mul x a) :=
  Subset_trans (le_add_zero_left x) (mul_mono_right_sub x ha)

theorem le_pow_aux_right (a b : PreOrd.{u}) (ha : Subset (succ zero) a) : Subset (succ zero) (pow_aux a b) := by
  induction b with
  | zero => exact Subset_refl _
...
mutual
  def pow_aux_mono_right_sub (a : PreOrd.{u}) (ha : Subset (succ zero) a) {b1 b2 : PreOrd.{u}} (h : Subset b1 b2) : Subset (pow_aux a b1) (pow_aux a b2) :=
    match b1, b2, h with
    | _, b, .zero_subset _ => le_pow_aux_right a b ha
    | _, _, .succ_subset hmem => pow_aux_mono_right_mem_mul a ha hmem
    | _, b, .sup_subset hsub => union_subset (le_pow_aux_right a b ha) (Subset.sup_subset fun n => pow_aux_mono_right_sub a ha (hsub n))

  def pow_aux_mono_right_mem_mul (a : PreOrd.{u}) (ha : Subset (succ zero) a) {b1 b2 : PreOrd.{u}} (h : Mem b1 b2) : Subset (mul (pow_aux a b1) a) (pow_aux a b2) :=
    match b1, b2, h with
    | _, _, .mem_succ hsub => mul_subset_mul (pow_aux_mono_right_sub a ha hsub) (Subset_refl a)
    | _, _, .mem_sup n hmem => Subset_trans (Subset_sup (pow_aux_mono_right_mem_mul a ha hmem) n rfl) (subset_union_right _ _)
end

theorem pow_aux_respects_right (a : PreOrd.{u}) (ha : Subset (succ zero) a) {b1 b2 : PreOrd.{u}} (hb : Equiv b1 b2) : Equiv (pow_aux a b1) (pow_aux a b2) :=
  ⟨pow_aux_mono_right_sub a ha hb.left, pow_aux_mono_right_sub a ha hb.right⟩

theorem pow_aux_respects {a1 a2 b1 b2 : PreOrd.{u}} (ha1 : Subset (succ zero) a1) (ha2 : Subset (succ zero) a2) (ha : Equiv a1 a2) (hb : Equiv b1 b2) : Equiv (pow_aux a1 b1) (pow_aux a2 b2) :=
...
noncomputable def pow (a b : PreOrd.{u}) : PreOrd.{u} :=
  if Equiv a zero then
    if Equiv b zero then succ zero else zero
  else
    pow_aux a b

theorem pow_respects {a1 a2 b1 b2 : PreOrd.{u}} (ha : Equiv a1 a2) (hb : Equiv b1 b2) : Equiv (pow a1 b1) (pow a2 b2) := by
  unfold pow
...
def add (x y : Ordinal.{u}) : Ordinal.{u} :=
...
def mul (x y : Ordinal.{u}) : Ordinal.{u} :=
...
instance : Mul Ordinal.{u} where
  mul := mul

noncomputable def pow (x y : Ordinal.{u}) : Ordinal.{u} :=
...
noncomputable instance : Pow Ordinal.{u} Ordinal.{u} where
  pow := pow
