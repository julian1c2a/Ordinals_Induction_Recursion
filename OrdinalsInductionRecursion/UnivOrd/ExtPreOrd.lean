...
import OrdinalsInductionRecursion.UnivOrd.PreOrd
...
def mem_sup_cases {α : Type u} {F : α → PreOrd} {z : PreOrd} (hx : Mem z (sup F)) {motive : Mem z (sup F) → Prop}
  (g : ∀ a hx_mem, motive (.mem_sup a hx_mem)) : motive hx :=
  match hx with
  | .mem_sup a hx_mem => g a hx_mem
...
def union (x y : PreOrd.{u}) : PreOrd.{u} :=
  .sup (α := ULift.{u, 0} Bool) (fun b => match b.down with
    | true => x
    | false => y)

theorem union_mono_left {x₁ x₂ : PreOrd} (h : Subset x₁ x₂) (y : PreOrd) : Subset (union x₁ y) (union x₂ y) :=
  Subset.sup_subset fun b => match b.down with
    | true => Subset_trans h (Subset_sup (Subset_refl x₂) ⟨true⟩ rfl)
    | false => Subset_sup (Subset_refl y) ⟨false⟩ rfl

theorem union_mono_right (x : PreOrd) {y₁ y₂ : PreOrd} (h : Subset y₁ y₂) : Subset (union x y₁) (union x y₂) :=
  Subset.sup_subset fun b => match b.down with
    | true => Subset_sup (Subset_refl x) ⟨true⟩ rfl
    | false => Subset_trans h (Subset_sup (Subset_refl y₂) ⟨false⟩ rfl)
