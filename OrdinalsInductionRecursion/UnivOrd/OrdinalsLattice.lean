...
theorem inter_comm (x y : Ordinal.{u}) : inter x y = inter y x :=
...
theorem le_total (x y : Ordinal.{u}) : x ≤ y ∨ y ≤ x :=
  Quotient.inductionOn₂ x y fun _ _ => PreOrd.le_total _ _

theorem inter_union_distrib_left (x y z : Ordinal.{u}) : inter x (union y z) = union (inter x y) (inter x z) := by
