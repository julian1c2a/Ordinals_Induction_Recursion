...
/-- Árboles con ramificación controlada por el universo Dybjer (UCodeFam).
La base para los Dybjer Sets. -/
inductive Tree : Type 1 where
  | zero : Tree
  | succ : Tree → Tree
  | sup  : {A : Type} → (c : UCodeFam A) → (A → Tree) → Tree
...
mutual
  ...
  inductive Subset : Tree → Tree → Prop where
    ...
    | sup_subset {A : Type} {c : UCodeFam A} {f : A → Tree} {y : Tree} : (∀ a, Mem (f a) y) → Subset (.sup c f) y
  ...
  inductive Mem : Tree → Tree → Prop where
    ...
    | mem_sup {x : Tree} {A : Type} {c : UCodeFam A} {f : A → Tree} (a : A) : Subset x (f a) → Subset (f a) x → Mem x (.sup c f)
end
...
theorem mem_sup_equiv {x : Tree} {A : Type} {c : UCodeFam A} {f : A → Tree} (a : A) (h : Equiv x (f a)) : Mem x (.sup c f) :=
...
def trans_all (y : Tree) :
  ...
  let mem_sub : ∀ {x z}, Mem x y → Subset y z → Mem x z := fun {_ z} h1 h2 =>
    ...
    | _, _, @Subset.sup_subset A c f _ hsub2 =>
      match h1 with
      | @Mem.mem_sup _ _ _ _ a hx_fa hfa_x => (trans_all (f a)).2.2 hfa_x hx_fa (hsub2 a)
  ...
  let sub_sub : ∀ {x z}, Subset x y → Subset y z → Subset x z := fun {x _} h1 h2 =>
    ...
    | _, @Subset.sup_subset A c f _ hsub => .sup_subset fun a => mem_sub (hsub a) h2
  ...
  let eq_mem : ∀ {x z}, Subset y x → Subset x y → Mem y z → Mem x z := fun {_ z} hyx hxy hyz =>
    ...
    | _, @Mem.mem_sup _ A c g a hyg hgy =>
...
/-- Conjuntos de Dybjer como cociente extensional -/
abbrev DSet := Quotient Tree.Setoid
